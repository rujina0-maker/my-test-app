...
export default function TarotApp() {
  ...
  const handleShuffle = () => {
    ...
    // 이미지 프리로드 - 백그라운드에서 미리 다운로드
    cards.forEach(card => {
      const img = new Image();
      img.src = CARD_IMAGES[card.id];
    });
    setTimeout(() => {
      setDrawnCards(cards);
      setShuffling(false);
  ...
  const handleReveal = (idx) => {
    ...
    if (newRevealed.length === drawnCards.length) {
      setTimeout(() => setStep("result"), 1200); // 이미지 로딩 대기 시간 늘림
    }
  };
                  ...
                  <div style={{ display: "flex", gap: "14px", alignItems: "center", marginBottom: "12px" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "baseline", gap: "8px", flexWrap: "wrap" }}>
                        <span style={{ fontSize: "1.2rem", fontWeight: 900, color: "#ffffff", fontFamily: "'Noto Serif KR', serif" }}>{card.name}</span>
                        <span style={{ fontSize: "0.8rem", color: card.reversed ? "#ffcccc" : "#99ddff", fontFamily: "'Noto Sans KR'", fontWeight: 600 }}>{meaning.title}</span>
                      </div>
                    </div>
                  </div>
                  {/* 구분선 */}
